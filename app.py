...
data = load_dati_comunali("dmodolomiti-turismo-veneto/dati-mensili-per-comune")
provincia = load_provincia_belluno("dmodolomiti-turismo-veneto/dati-provincia-annuali")
...
# ======================
if data.empty:
    st.error("❌ Nessun dato comunale caricato. Controlla la cartella 'dmodolomiti-turismo-veneto/dati-mensili-per-comune'.")
    st.stop()
else:
...
# ======================
# 📋 CONFRONTO TRA ANNI E MESI (COMUNI)
# ======================
...
if not df_filtered.empty and len(anno_sel) >= 1:
    ...
    if len(anno_sel) == 2:
        ...
        anno_prev, anno_recent = anni_sorted[0], anni_sorted[1]
...
if mostra_stl:
    ...
    if stl_data.empty:
        ...
        st.markdown("---")
        st.header(f"🌄 STL {stl_tipo} – Arrivi e Presenze mensili")
        ...
        st.subheader("📈 Andamento Arrivi Mensili")
        fig_arr_stl = px.line(stl_filtrata, x="mese", y="arrivi", color="anno", markers=True)
        st.plotly_chart(fig_arr_stl, use_container_width=True)
